...
def test_operating_layer_obsidian_cards_route_returns_helper_payload(
    ...
    monkeypatch.setattr(operating_layer_server, "fetch_obsidian_cards_payload", lambda: expected)
...
def test_operating_layer_obsidian_task_create_route_creates_task_and_event(tmp_path: Path) -> None:
    ...
    events = [
        json.loads(line)
        for line in (tmp_path / ".devflow" / "tasks" / "task-0001" / "events.jsonl").read_text(encoding="utf-8").splitlines()
    ]
