...
from devflow.legacy.control_room.loop_promotion import handle_promotion_candidate, promotion_candidates
from devflow.legacy.control_room.parallel_task_creation import ParallelTaskCreationRun, run_parallel_task_creation_batch
from devflow.legacy.control_room.parallel_verification import ParallelVerificationRun, run_parallel_verification_batch
from devflow.legacy.control_room.parallel_worker import ParallelWorkerRun, run_parallel_worker_batch
...
from devflow.legacy.control_room.promotion import main_checkout_has_uncommitted_changes
from devflow.legacy.control_room.question_resume import build_question_snapshot
...
def run_loop(
    ...
    *,
    ...
    for iteration_number in range(1, iterations_limit + 1):
        ...
        candidates = promotion_candidates(root, report)
        if candidates:
            status, stop_reason, next_action, preview_payload, promotion_payload = handle_promotion_candidate(
                root,
                loop_id=definition.loop_id,
                actions=definition.actions,
                allow_loop_promotion=definition.policy.allow_promotion,
                allow_high_risk=definition.policy.allow_high_risk,
                loop_config_path=loop_config_path(root, definition.loop_id),
                candidate=candidates[0],
                allow_promote=allow_promote,
            )
            signal = _signal(status, stop_reason, next_action)
            preview = LoopPromotionPreviewResult(**preview_payload) if preview_payload is not None else None
            promotion = LoopPromotionResult(**promotion_payload) if promotion_payload is not None else None
