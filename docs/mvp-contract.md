...
`devflow maintenance reset-dogfood-state --preview` reports only allowlisted ignored runtime artifacts under `.devflow`; `--yes` removes them. The allowlist covers `.devflow/tasks/task-*`, `.devflow/workspaces/task-*`, `.devflow/worktrees/task-*`, `.devflow/prune-runs/`, `.devflow/dogfood/`, `.devflow/agent-runs/`, and runtime knowledge/outcome/release logs. The command preserves tracked seed/config/context files and refuses symlink/path escapes outside `.devflow`. `devflow maintenance repair-state --preview` reports missing task baseline artifacts; `--yes` recreates only missing baseline files without overwriting existing evidence. `doctor` reports missing task baseline artifacts read-only.
