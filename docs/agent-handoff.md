...
Milestone 24 Intent-To-Goal/Task Scaffold is selected as the approved next implementation slice. It should convert raw operator requests into reviewable Idea Foundry and goal/task scaffold evidence with explicit approval before canonical goal/task creation. Artifacts:
...
Local `main` contains the Milestone 23 implementation and release-readiness evidence has passed. The next safe action is to create the Milestone 24 implementation task and start Task 1 from the plan: characterize current intent-to-goal behavior with failing focused tests.
