...
Local-worker selection is opt-in and visible-Qwen-worker-first for current sessions; see
[docs/local-worker-policy.md](local-worker-policy.md). In Codex, the current
workflow is a visible `qwen36_27b_mtp_coder` subagent spawn; in Hermes,
`hermes-qwen-mtp` mirrors the same bounded packet semantics. Legacy Dev-Flow
local Ollama advisory and patch proposal commands remain product evidence
surfaces, not automatic local-worker routing defaults.
