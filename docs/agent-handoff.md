...
Milestone 25 Stop The Task/Data Sprawl is implemented as the prior hardening baseline. It treats current task/workspace/worktree history as disposable dogfood clutter, adds `devflow maintenance reset-dogfood-state` and `devflow maintenance repair-state`, makes task baseline artifacts complete at creation, runs task-producing dogfood cases in scratch projects by default, keeps only the latest dogfood run report unless `--keep-runs` is explicit, keeps prune previews read-only, and makes dashboard/next report a clean control room after reset.
