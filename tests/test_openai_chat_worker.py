...
def test_get_openai_chat_worker_adapter() -> None:
    adapter = get_worker_adapter("openai_chat")
    assert isinstance(adapter, OpenAIChatWorkerAdapter)
    assert adapter.name == "openai_chat"
