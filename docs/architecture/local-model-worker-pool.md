...
| `gemma4-review:latest` (alias of `gemma4:31b`) | `gemma-dense-judge` | `mac_studio` | `heavy` | strict local reviewer/final judge | instruction-following audit, UX/spec verification, multimodal artifact review if manifest supports it | use as judge/reviewer, not a default implementation worker; tuned alias constrains context to reduce local memory pressure | `ollama show gemma4-review:latest` |

Public model context can inform starter assumptions, but it is not authority. Google/Ollama list Gemma 4 tags including `gemma4:26b` and `gemma4:31b`; `gemma4:31b` is the dense 31B model and `gemma4:26b` is the 26B A4B MoE variant. The local dense-judge profile routes through `gemma4-review:latest`, a tuned alias of `gemma4:31b` with `num_ctx 32768`, so review runs do not accidentally request the parent model's full 262144-token context. Model names are never enough: Dev-Flow should prefer actual `ollama show` manifests whenever available.
