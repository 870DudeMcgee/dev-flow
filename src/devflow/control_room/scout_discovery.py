"""Deterministic discovery for scout-first agent workflow packets."""
...
VALID_AGENT_SCOUT_LANES = {"direct_tiny_edit", "deterministic_tool", "builder", "judge", "ask_user"}
...
def _extract_path(text: str) -> str | None:
    ...
    match = re.search(r"((?:src|tests|docs|\.devflow|scripts)/[^\s,;)]+|[A-Z_]+\.md|AGENTS\.md|CONTEXT\.md)", cleaned)
    if match:
...
def parse_handoff_doc(root: Path | str, handoff: str | None) -> HandoffContext:
    ...
    target_files = [target for item in _bullet_lines(_section(markdown, "Target files")) if (target := _extract_path(item))]
    ...
    return HandoffContext(_relative(repo_root, path), True, title, task_text, target_files, tests, verification, constraints)
...
def _task_scope_text(root: Path, scout: RepoScout, task_id: str) -> tuple[str, str, str | None]:
    """Return task-derived title, description, and verification when a DevFlow task exists."""
    try:
        task = get_task(root, task_id)
    except KeyError:
        return "", "", None
    description_parts = [part for part in [task.definition_of_done, scout.get_task_description(task_id)] if part]
    return task.title, "\n".join(description_parts), task.verification_command
...
def _risks(context: HandoffContext, files_to_touch: list[str], tests: list[str], freshness: dict[str, str]) -> list[str]:
    risks: list[str] = []
...
def discover_agent_scout_context(
    ...
    *,
    ...
    task_title, task_description, task_verification = _task_scope_text(repo_root, scout, task_id)
    ...
    if not discovered_files:
        discovered_files.extend(_relative(repo_root, path) for path in scout.get_referenced_files(scope_title, description))
    resolved_files = _unique(files_to_touch or discovered_files)

    relevant_paths = _existing_paths(repo_root, resolved_files)
    discovered_tests = [*_extract_pytest_tests(context.verification), *(_relative(repo_root, path) for path in scout.get_test_files(relevant_paths))]
    ...
    if not has_scope or not resolved_files:
        ...
        lane = "ask_user" if recommended_lane in {None, "auto"} else _lane(resolved_files, resolved_tests, resolved_risks, recommended_lane)
    else:
