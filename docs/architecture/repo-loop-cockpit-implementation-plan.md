...
Start RLC-03 by narrowing the Obsidian task bridge from broad card browsing to curated packet intake. The route should accept a narrow packet (source, repo, intent, constraints, acceptance criteria, suggested preset) and optionally create a pipeline run — but must not launch Hermes.
