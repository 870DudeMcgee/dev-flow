...
_SPEC = ProviderWorkerSpec(
    ...
    header_label="OpenAI Chat Worker Execution",
    default_model="gpt-4o",
    default_base_url="https://api.openai.com/v1",
    default_api_key_env="OPENAI_API_KEY",
    default_api_key_missing_summary=(
        "OpenAI Chat provider API key is not configured. Set api_key_env in provider "
        "config and export the matching environment variable."
    ),
    api_log_label="OpenAI Chat API",
    connection_error_target="OpenAI Chat agent",
    execution_error_prefix="OpenAI Chat execution",
)
