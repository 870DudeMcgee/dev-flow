...
The first execution helper for verification batches is task-grained. It runs one projected batch at a time, groups commands by task, folds multiple commands for the same task into one shell script, and starts up to a bounded `max_parallel` number of task verification subprocesses. Each subprocess still flows through the existing `verify_task` path, task-local mutation lock, process-group cleanup, `logs/verify.log`, `verification.json`, and task events. The helper writes a compact derived run report under `.devflow/freshness/verification-runs/`; that report is evidence about the batch run, not a goal-completion certificate.
