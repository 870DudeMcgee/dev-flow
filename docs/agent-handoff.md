...
Milestone 25 Stop The Task/Data Sprawl is the active hardening slice. It treats current task/workspace/worktree history as disposable dogfood clutter, adds `devflow maintenance reset-dogfood-state` and `devflow maintenance repair-state`, makes task baseline artifacts complete at creation, runs task-producing dogfood cases in scratch projects by default, keeps prune previews read-only, and makes dashboard/next report a clean control room after reset.

## Known Worktree State At Handoff

Local `main` contains Milestone 24. The current active worktree contains Milestone 25 implementation changes pending verification and review.
