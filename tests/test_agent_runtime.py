...
def test_resolve_builtin_shell_alias_is_task_run_runtime(tmp_path: Path) -> None:
    runtime = resolve_agent_runtime(tmp_path, "devflow-shell-worker")

    assert runtime.agent_id == "devflow-shell-worker"
    assert runtime.adapter == "shell"
    assert runtime.adapter_maturity == "stable_runtime"
    assert runtime.execution_surface == "task_run"
    assert runtime.task_run_allowed is True
    assert runtime.agent_run_allowed is False
    ...
    assert runtime.remote_provider is False
    assert runtime.refusal_reason is None
    assert runtime.next_command == "devflow task run <task-id> --worker devflow-shell-worker -- <command>"
...
def test_resolve_disabled_agent_is_not_runnable_or_packetable(tmp_path: Path) -> None:
    ...
    runtime = resolve_agent_runtime(tmp_path, "disabled-shell")
