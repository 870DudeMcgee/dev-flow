...
class AnthropicMessagesWorkerAdapter:
    ...
    def run(self, worker_input: WorkerInput) -> WorkerResult:
        ...
        with worker_input.log_file.open("w", encoding="utf-8") as log:
            log.write(f"=== Anthropic Worker Execution for Task {worker_input.task_id} ===\n")
            log.flush()
        ...
        if api_key_env:
            ...
            if not api_key:
                err_msg = "Anthropic provider API key is not configured. Set api_key_env in provider config and export the matching environment variable."
                with worker_input.log_file.open("a", encoding="utf-8") as log:
                    log.write(f"{err_msg}\n")
                    log.flush()
                return WorkerResult(
        ...
        # 4. Invoke Anthropic Messages API
        ...
        with worker_input.log_file.open("a", encoding="utf-8") as log:
            log.write(f"Connecting to Anthropic API on {url} (model: {model}, timeout: {timeout}s)...\n")
            log.flush()
        ...
        try:
            ...
            err_msg = f"Anthropic execution encountered an exception: {e}"
            with worker_input.log_file.open("a", encoding="utf-8") as log:
                log.write(f"{err_msg}\n")
                log.flush()
            return WorkerResult(
