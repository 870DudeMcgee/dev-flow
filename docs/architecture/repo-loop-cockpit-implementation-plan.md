...
Start RLC-05 by adding a supervisor intent summary artifact. Generate or import an intent summary (user wants, product outcome, non-negotiables, misunderstandings, what done feels like) that lives as a pipeline run artifact before readiness validation. Files: `pipeline_intent.py` or folded into `pipeline_run.py`, Brainstorm/packet tests.
