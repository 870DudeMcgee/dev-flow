...
It is not the coding intelligence itself. It is the operational layer around coding intelligence: task state, isolated workspaces, locks and ownership, status, logs, verification evidence, and gated promotion.

Workers are replaceable. The stable code-changing runtime supports shell workers, while the manual proof-agent handoff and legacy local Ollama advisory ladder produce evidence only. The registry-backed `devflow task run <task-id> --worker qwopus-implementer` path is the narrow first local Ollama patch-proposal runtime: it writes `proposal.patch` evidence for Dev-Flow to apply and verify. The local model worker pool adds registry-visible read-only profiles for local Ollama fleets and writes generalized WorkerEvidence under task-local `local-model-runs/`. New model profiles are added through `devflow agent catalog`, `devflow agent add-provider`, and `devflow agent add-model`, not by editing Python builtins or hand-authoring repeated YAML blocks. Remote provider profiles are registry-visible evidence lanes for explicit advisory reports and explicit patch proposals; they are not executable `task run` workers. Remote provider task-run adapters, autonomous routing, and broader orchestration remain non-stable until explicitly promoted through the registry and adapter-runtime sequence.
...
- **Operating Layer**: `devflow operating-layer snapshot --json`, `devflow operating-layer serve`
...
These commands are read-only or manual planning aids except for explicit evidence-writing model calls and approval-gated provider/profile onboarding. Milestone 17 promotes task-fit, scout, route, and scorecard evidence commands into the stable contract without provider calls. `agent advise` and `agent propose-patch` are explicit configured-provider evidence lanes that write bounded reports or patch-proposal artifacts; they do not make autonomous routing decisions, run workers, apply patches, verify, promote, commit, or push.

The current control-room MVP intentionally excludes remote provider task-run adapters, autonomous browser/web mutation surfaces, database state, autonomous scheduling/routing, and provider-backed worktree orchestration. The bounded exception is Dev-Flow-owned configured-provider evidence: advisory reports under `.devflow/reports/agent-advisory-runs/` or task-local advisory runs, plus explicit patch-proposal evidence that still must pass `review-patch`, `patch-dry-run`, `apply-patch`, verification, and promotion gates. The approved browser surface is the local operating layer over filesystem evidence: `devflow operating-layer snapshot --json` and `devflow operating-layer serve`, documented in [docs/architecture/local-operating-layer-ui.md](docs/architecture/local-operating-layer-ui.md). The practical local model pool is documented in [docs/architecture/local-model-worker-pool.md](docs/architecture/local-model-worker-pool.md). Hermes integration is documented as an external operator/chat gateway over existing supervisor-safe commands, not as a Dev-Flow runtime, in [docs/integrations/hermes-operator-layer.md](docs/integrations/hermes-operator-layer.md), [docs/integrations/hermes-command-allowlist.md](docs/integrations/hermes-command-allowlist.md), [docs/integrations/hermes-imessage-exploration.md](docs/integrations/hermes-imessage-exploration.md), [docs/integrations/hermes-local-parallelism.md](docs/integrations/hermes-local-parallelism.md), [docs/integrations/hermes-worker-evidence-synthesis.md](docs/integrations/hermes-worker-evidence-synthesis.md), and the setup rollout in [docs/integrations/hermes-telegram-mac-mini-rollout.md](docs/integrations/hermes-telegram-mac-mini-rollout.md). An opt-in Git-native shell-worker isolation and promotion slice is available through `devflow task create --git-worktree`, documented in [docs/architecture/git-native-worker-isolation-and-promotion.md](docs/architecture/git-native-worker-isolation-and-promotion.md). Registry, adapter-runtime, and evidence-only task-fit/context-routing boundaries are documented in [docs/architecture/agent-registry-and-adapter-runtime.md](docs/architecture/agent-registry-and-adapter-runtime.md) and [docs/architecture/agent-selection-and-context-routing.md](docs/architecture/agent-selection-and-context-routing.md).
...
.devflow/
  tasks/<task-id>/
    ...
    agent-advisory-runs/<run-id>/prompt.md
    agent-advisory-runs/<run-id>/response.md
    agent-advisory-runs/<run-id>/response.raw.json
    agent-advisory-runs/<run-id>/run.json
    logs/
      worker.log
      verify.log
    idea.md
  ...
  reports/agent-advisory-runs/<run-id>/prompt.md
  reports/agent-advisory-runs/<run-id>/response.md
  reports/agent-advisory-runs/<run-id>/response.raw.json
  reports/agent-advisory-runs/<run-id>/run.json
...
- `devflow agent add-provider` and `devflow agent add-model` are the supported onboarding path for provider configs and model profiles. Dry-run mode writes nothing; real runs write only `.devflow/providers/<provider-id>.yaml` or `.devflow/agents/registry.yaml` after validation.
- `devflow agent advise --profile deepseek-v4-flash-planner --job gap-analysis --json` and related advisory jobs call only the configured provider when explicitly invoked, require the configured API-key environment variable, and write prompt/response/run evidence with `will_*: false` safety flags. Dry-run mode writes no evidence and makes no provider call.
- `devflow agent propose-patch --task <task-id> --profile deepseek-v4-pro-patch-proposer --json` is an explicit patch-proposal lane. It writes proposal evidence only and never applies the patch; normal patch review, dry-run, apply, verification, and promotion gates still control source changes.
...
6. **Remote Providers Non-Stable As Workers**: Remote provider adapters are not stable executable production runtimes. Standard task execution refuses to run them; configured-provider profiles are limited to explicit advisory reports or patch-proposal evidence.
