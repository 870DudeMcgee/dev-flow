...
from devflow.legacy.control_room.operating_layer_architecture_evidence_script import ARCHITECTURE_EVIDENCE_JS
from devflow.legacy.control_room.operating_layer_idea_greenhouse_script import IDEA_GREENHOUSE_JS
from devflow.legacy.control_room.operating_layer_obsidian_intake_script import OBSIDIAN_INTAKE_JS
from devflow.legacy.control_room.operating_layer_model_picker_script import MODEL_PICKER_JS
from devflow.legacy.control_room.operating_layer_pipeline_script import PIPELINE_JS
from devflow.legacy.control_room.operating_layer_workbench_script import WORKBENCH_JS
...
let obsidianIntake = { status: 'loading', payload: null };
let selectedObsidianCardId = null;
...
""" + MODEL_PICKER_JS + """// === BRAINSTORM SESSION MANAGEMENT ===
...
function setupTaskSurfaceActions() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    const obsidianCardButton = e.target.closest('[data-obsidian-card-id]');
    if (obsidianCardButton) {
      e.preventDefault();
      e.stopPropagation();
      selectedObsidianCardId = obsidianCardButton.dataset.obsidianCardId || null;
      renderObsidianIntake();
      return;
    }

    const obsidianContextButton = e.target.closest('[data-obsidian-use-context]');
    if (obsidianContextButton) {
      e.preventDefault();
      e.stopPropagation();
      const card = obsidianCards().find(item => item.id === obsidianContextButton.dataset.obsidianUseContext);
      const input = $('brainstorm-message');
      const status = $('obsidian-intake-action-status');
      if (!card || !input) return;
      input.value = buildObsidianBrainstormContext(card);
      input.dispatchEvent(new Event('input', { bubbles: true }));
      input.focus();
      if (status) status.textContent = 'Context copied to Brainstorm.';
      return;
    }
    ...
    if (await handleIdeaGreenhouseClick(e)) return;
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (handleIdeaGreenhouseKeydown(e)) return;
  });
}
