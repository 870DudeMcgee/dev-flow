...
def test_gemini_worker_success(tmp_path: Path) -> None:
    ...
    task_dir = tmp_path / ".devflow" / "tasks" / "task-gemini-1"
    task_dir.mkdir(parents=True)
    (task_dir / "logs").mkdir(parents=True)

    # Write task.yaml
    (task_dir / "task.yaml").write_text("id: task-gemini-1\ntitle: Test Task\nstatus: created\n", encoding="utf-8")

    # Set up workspace (copy hello.txt to workspace)
    workspace_path = tmp_path / ".devflow" / "workspaces" / "task-gemini-1"
    ...
    # Mock Gemini API response returning a clean ready diff
    ...
    mock_response.read.return_value = json.dumps(candidates_message).encode("utf-8")

    adapter = GeminiWorkerAdapter()

    # Stub build_context_pack to return our dummy pack
    with patch("urllib.request.urlopen") as mock_urlopen, \
         patch("devflow.control_room.gemini_worker.build_context_pack", return_value=context_pack_data):
...
def test_gemini_worker_connection_failure(tmp_path: Path) -> None:
    # Set up task directories
    task_dir = tmp_path / ".devflow" / "tasks" / "task-gemini-2"
    task_dir.mkdir(parents=True)
    (task_dir / "logs").mkdir(parents=True)

    workspace_path = tmp_path / ".devflow" / "workspaces" / "task-gemini-2"
    workspace_path.mkdir(parents=True)
    ...
    with patch("urllib.request.urlopen", side_effect=urllib.error.URLError("Connection refused")), \
         patch("devflow.control_room.gemini_worker.build_context_pack", return_value={}):
        result = adapter.run(worker_input)

        assert result.status == "worker_failed"
        assert "Error connecting to Gemini agent" in result.summary
        assert result.exit_code == 1
        assert log_file.exists()


def test_gemini_worker_blocked_status(tmp_path: Path) -> None:
    # Set up task directories
    task_dir = tmp_path / ".devflow" / "tasks" / "task-gemini-3"
    task_dir.mkdir(parents=True)
    (task_dir / "logs").mkdir(parents=True)

    workspace_path = tmp_path / ".devflow" / "workspaces" / "task-gemini-3"
    workspace_path.mkdir(parents=True)
