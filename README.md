...
Workers are replaceable. The stable code-changing runtime supports shell workers, while the manual proof-agent handoff and legacy local Ollama advisory ladder produce evidence only. The registry-backed `devflow task run <task-id> --worker qwopus-implementer` path is the narrow first local Ollama patch-proposal runtime: it writes `proposal.patch` evidence for Dev-Flow to apply and verify. The local model worker pool adds registry-visible read-only profiles for Josh's heterogeneous Mac mini/Mac Studio Ollama fleet and writes generalized WorkerEvidence under task-local `local-model-runs/`. OpenRouter-backed DeepSeek profiles are registry-visible evidence lanes for explicit advisory reports and explicit patch proposals; they are not executable `task run` workers. Remote provider task-run adapters, autonomous routing, and broader orchestration remain non-stable until explicitly promoted through the registry and adapter-runtime sequence.
...
- `devflow agent advise --profile deepseek-v4-flash-planner --job gap-analysis --json` and related DeepSeek advisory jobs call OpenRouter only when explicitly invoked, require `OPENROUTER_API_KEY` from the environment, and write prompt/response/run evidence with `will_*: false` safety flags. Dry-run mode writes no evidence and makes no provider call.
...
6. **Remote Providers Non-Stable As Workers**: Remote provider adapters are not stable executable production runtimes. Standard task execution refuses to run them; the only current remote-provider surface is explicit OpenRouter DeepSeek evidence generation for advisory reports or patch proposals.
