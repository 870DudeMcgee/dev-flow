...
Start RLC-04 by adding a brainstorm-to-classification gate. The brainstorm pipeline should write `classification.json` with rationale, deterministic-tool eligibility, and eligible presets before proceeding to task creation. Files: `brainstorm_pipeline.py`, `brainstorm.py`, `operating_layer_brainstorm_handlers.py`.
