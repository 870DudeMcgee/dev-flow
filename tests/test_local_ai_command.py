...
def test_local_ai_run_worker_wave_dry_run_json(
    ...
    result = runner.invoke(app, ["local-ai", "run-worker-wave", "wave.json", "--dry-run", "--concurrency", "1", "--json"])
...
def test_local_ai_run_worker_wave_rejects_non_one_concurrency(
    tmp_path: Path,
) -> None:
    wave_path = tmp_path / "wave.json"
    wave_path.write_text("[]", encoding="utf-8")

    result = runner.invoke(app, ["local-ai", "run-worker-wave", "wave.json", "--concurrency", "2", "--dry-run", "--json"])
