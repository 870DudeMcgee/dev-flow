...
def test_get_openai_compatible_worker_adapter() -> None:
    adapter = get_worker_adapter("openai_compatible")
    assert isinstance(adapter, OpenAICompatibleWorkerAdapter)
    assert adapter.name == "openai_compatible"


def test_openai_compatible_worker_success(tmp_path: Path) -> None:
    ...
    task_dir = tmp_path / ".devflow" / "tasks" / "task-openai-1"
    task_dir.mkdir(parents=True)
    (task_dir / "logs").mkdir(parents=True)

    # Write task.yaml
    (task_dir / "task.yaml").write_text("id: task-openai-1\ntitle: Test Task\nstatus: created\n", encoding="utf-8")

    # Set up workspace (copy hello.txt to workspace)
    workspace_path = tmp_path / ".devflow" / "workspaces" / "task-openai-1"
    ...
    with patch("urllib.request.urlopen") as mock_urlopen, \
         patch("devflow.control_room.openai_compatible_worker.build_context_pack", return_value=context_pack_data):
...
def test_openai_compatible_worker_connection_failure(tmp_path: Path) -> None:
    # Set up task directories
    task_dir = tmp_path / ".devflow" / "tasks" / "task-openai-2"
    task_dir.mkdir(parents=True)
    (task_dir / "logs").mkdir(parents=True)

    workspace_path = tmp_path / ".devflow" / "workspaces" / "task-openai-2"
    workspace_path.mkdir(parents=True)
    ...
    with patch("urllib.request.urlopen", side_effect=urllib.error.URLError("Connection refused")), \
         patch("devflow.control_room.openai_compatible_worker.build_context_pack", return_value={}):
        result = adapter.run(worker_input)

        assert result.status == "worker_failed"
        assert "Error connecting to OpenAI-compatible agent" in result.summary
        assert result.exit_code == 1
        assert log_file.exists()


def test_openai_compatible_worker_blocked_status(tmp_path: Path) -> None:
    # Set up task directories
    task_dir = tmp_path / ".devflow" / "tasks" / "task-openai-3"
    task_dir.mkdir(parents=True)
    (task_dir / "logs").mkdir(parents=True)

    workspace_path = tmp_path / ".devflow" / "workspaces" / "task-openai-3"
    workspace_path.mkdir(parents=True)
