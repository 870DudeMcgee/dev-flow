...
from devflow.legacy.control_room.goal_loop import GoalVerificationBatch, GoalVerificationItem
...
class VerificationBatchSelectionError(ValueError):
    pass


def run_projected_verification_batch(
    root: Path,
    goal_id: str,
    batch_id: str,
    *,
    ...
    """Run one verification batch currently projected by the bounded freshness loop."""
    ...
    if report.status == "needs_human_decision":
        raise VerificationBatchSelectionError(
            "Freshness loop needs a human decision before verification dispatch. "
    ...
    for goal in report.goal_loop:
        ...
        for batch in goal.verification_batches:
            if batch.batch_id == batch_id:
                return run_parallel_verification_batch(
                    root,
                    batch,
                    max_parallel=max_parallel,
                    timeout_seconds=timeout_seconds,
                )
        available = ", ".join(batch.batch_id for batch in goal.verification_batches) or "none"
        raise VerificationBatchSelectionError(
            f"Verification batch {batch_id!r} is not projected for goal {goal_id!r}. Available batches: {available}."
        )
...
def render_parallel_verification_run(run: ParallelVerificationRun) -> str:
    lines = [
        f"Verification batch: {run.batch_id}",
        f"Status: {run.status}",
    ...
    for result in run.results:
        ...
        if result.verification_log_path:
            lines.append(f"  log: {result.verification_log_path}")
        if result.error:
